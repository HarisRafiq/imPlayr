...
- (id)initWithAsyncSocket:(GCDAsyncSocket *)newSocket configuration:(HTTPConfig *)aConfig
{
	if ((self = [super init]))
	{
		...
		responseDataSizes = [[NSMutableArray alloc] initWithCapacity:10];
	}
	return self;
}
...
- (void)redirectoTo:(NSString*)path
{
	...
	CFHTTPMessageSetHeaderFieldValue(response, CFSTR("Content-Length"), (CFStringRef)CFBridgingRetain(length));
	CFHTTPMessageSetHeaderFieldValue(response, CFSTR("Content-Type"), CFSTR("text/html; charset=utf-8"));
	CFHTTPMessageSetHeaderFieldValue(response, CFSTR("Location"), (CFStringRef)CFBridgingRetain(path));
	NSData *responseData = (NSData *)CFBridgingRelease(CFHTTPMessageCopySerializedMessage(response));
...
- (void)sendString:(NSString*)text mimeType:(NSString*)mimeType
{
	...
	NSData *responseData = [self preprocessResponse:  response  ];
	[asyncSocket writeData:responseData withTimeout:TIMEOUT_WRITE_HEAD tag:HTTP_FINAL_RESPONSE];
	
	[asyncSocket writeData:[content dataUsingEncoding:NSUTF8StringEncoding] withTimeout:TIMEOUT_WRITE_BODY tag:HTTP_FINAL_RESPONSE];
