...
- (void)connectionDidDie:(NSNotification *)notification
{
    ...
    if  ([self numberOfHTTPConnections]<=1)
        [[imStreamer sharedInstance] setChanged  :NO];
...
- (NSInteger)numberOfFiles{return [controller numberOfFiles];}

// the file name by the index
- (NSString*)fileNameAtIndex:(NSInteger)index{return [controller fileNameAtIndex:index]; }

// provide full file path by given file name
- (NSString*)filePathForFileName:(NSString*)filename{return [controller filePathForFileName:filename];}



// implement this method to delete requested file and update the file list
- (void)fileShouldDelete:(NSString*)fileName{[controller fileShouldDelete:fileName];}
@end
