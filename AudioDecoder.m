...
@synthesize lpcmBuffer = _lpcmBuffer;
                          ...
                          bufferSize:(NSUInteger)bufferSize
{
    ...
    if (self) {
        ...
        _lpcmBuffer = [[LPCBuffer alloc] init];
...
{
    if (!_decodingContextInitialized) {
        return AudioDecoderFailed;
    }
    
    pthread_mutex_lock(&_decodingContext.mutex);
    
    AudioFile *audioFile = [_playbackItem audioFile];
    if ([audioFile isFailed]) {
        [_lpcmBuffer setEnd:YES];
        pthread_mutex_unlock(&_decodingContext.mutex);
        return AudioDecoderFailed;
    }
    if (![audioFile isFinished]) {
        NSUInteger dataOffset = [_playbackItem dataOffset];
        NSUInteger expectedDataLength = [audioFile expectedLength];
        NSInteger receivedDataLength  = (NSInteger)([audioFile receivedLength] - dataOffset);
        ...
        double downloadTime = 1000.0 * (bytesPerRead - (receivedDataLength - packetDataOffset)) / [audioFile downloadSpeed];
    ...
    if (ioOutputDataPackets == 0) {
        [_lpcmBuffer setEnd:YES];
        pthread_mutex_unlock(&_decodingContext.mutex);
        return AudioDecoderEndEncountered;
    }
    ...
    [_lpcmBuffer writeBytes:_decodingContext.outputBuffer length:inNumBytes];
    _decodingContext.outputPos += ioOutputDataPackets;
    
    pthread_mutex_unlock(&_decodingContext.mutex);
    return AudioDecoderSucceeded;
}
